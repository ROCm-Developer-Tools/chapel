...
/**
 * Example to demonstrate the performance difference between RMO and CA. 
 * 
 * The program performs a simple matrix copy operation. Source matrix is in RMO form. Destination matrix is in CA. 
 * CA performance is compared to an RMO-RMO copy. 
 *
 * The validation routine prints "PASS" if the copy is successful. 
 *
 * The GPU path is currently not functional. We are currently not able to generate code for GPU when using dmapped 
 * causes on arrays  
 * 
 * CA is a lot slower on CPU platforms
 */
...
pragma "request gpu offload"
proc ca(agent: string) {

  var domRowMajor : domain(2) = {rows, cols};
  var domCA : domain(2) dmapped CA(tilesize) = {rows, cols};
  var A : [domRowMajor] real; 
  var B : [domCA] real; 
