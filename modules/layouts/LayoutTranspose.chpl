...
/** 
 *  This module implements a domain map to transpose a multi-dimensional array. If the array is in
 *  RMO (default in Chapel) then Transpose() will convert the array to CMO. On the other hand, 
 *  a CMO array will be converted to RMO. 
 * 
 ...
 *
 *  To declare a Transpose domain, invoke the ``Transpose`` constructor without arguments in 
 *  a `dmapped` clause. For example:

  .. code-block:: chapel

    use LayoutTranspose;
    var D: domain(2) dmapped Transpose() = {rows, cols};
    var A: [D] real;  // array in CMO

  .. code-block:: chapel

 * 
 * Transpose expects arrays to be two-dimensional. Higher ranked arrays will be supported
 * in future revisions. 
 * 
 * Transpose does not support Associative, Opaque or Sparse domains. 
 *  
 */
class Transpose: BaseDist {

  proc dsiNewRectangularDom(param rank: int, type idxType, param stridable: bool) {
      return new TransposeDom(rank, idxType, stridable, this);
  }
 
  proc dsiClone() return this;
  
  proc dsiEqualDMaps(that: Transpose) param {
    return true;
  }
}
 ...
 * 
 */
class TransposeDom: BaseRectangularDom {
  param rank: int;
  type idxType;
  ...
  proc TransposeDom(param rank, type idxType, param stridable, dist) {
    if (rank != 2) then
      compilerError("Transpose currently only supports 2D rectangular domains"); 
    this.dist = dist;
    if chpl_dl_debug then 
      writeln("Transpose domain created.");
  }
  ...
  proc dsiBuildArray(type eltType) {
    return new TransposeArr(eltType = eltType, rank = rank, idxType = idxType, 
			stridable = stridable, dom = this);
  }

  proc dsiDisplayRepresentation() {
    writeln("Display representation of Transpose domain");
  }

  proc dsiSerialWrite(f) {
    if chpl_dl_debug then  {
      writeln("Serial write of Transpose domain");
    }
    writeln(ranges);
  }
  ...
  proc dsiBuildDom(param rank: int, type idxType, param stridable: bool,
		   ranges: rank*range(idxType,
				      ...
				      stridable)) {
    var dom = new TransposeDom(rank, idxType, stridable, dist);
    for i in 1..rank do
...
class TransposeArr : BaseArr {
  ...
  var dom : TransposeDom(rank = rank, idxType = idxType, stridable = stridable);
  ...
  proc TransposeArr(type eltType, param rank, type idxType, param stridable, dist) {
    writeln("basic array created");
  }
   ...
   * When are the different versions called? It appears only the second version is being called 
   */
  ...
  proc dsiSerialReadWrite(f) {
    ...
    if chpl_dl_debug then 
      writeln("Transposed array serial write"); 
    ...
    proc recursiveArrayWriter(in idx: rank*idxType, dim=rank, in last=false) {
	  ...
	  if chpl_dl_debug then 
	    writeln("stride = ", makeStridePositive, " ranges = ", dom.ranges(dim));
	  ...
	  // idx already transposed; reversing before sending to dsiAccess(). FIXME
	  var tmp_idx = idx;
	  transpose(tmp_idx);  
	  f <~> dsiAccess(tmp_idx);
        ...
        for j in dom.ranges(dim) by makeStridePositive {
	  if chpl_dl_debug then 
	    writeln("dim = ", makeStridePositive, " ranges = ", dom.ranges(dim));
  ...
  proc dsiSerialWrite(f) {
    if chpl_dl_debug then 
      writeln("Serial write of Transposed Arr : ");
